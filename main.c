...
int	test_strnstr(char *dest, char *src, int len, char *desc)
{
	...
	ft_ret = ft_strnstr(ft_dest, src, len);
	std_ret = strnstr(std_dest, src, len);
...
int	test_strlcpy(char *dest, char *src, size_t size, char *desc)
{
	char	ft_dest[256];
	char	std_dest[256];
	size_t	ft_ret;
	...
	std_ret = strlcpy(std_dest, src, size);
	printf("Test: %s\n", desc);
	printf("\tft_strlcpy returns:%zu, Result: %s\n", ft_ret, ft_dest);
	printf("\tstrlcpy returns:%zu, Result: %s\n\n", std_ret, std_dest);
...
int	test_strlcat(char *dest, char *src, size_t size, char *desc)
{
	char	ft_dest[256];
	char	std_dest[256];
	size_t	ft_ret;
...
int	test_strchr(char *src, char c, char *desc)
{
	...
	ft_ret = ft_strchr(ft_src, c);
	std_ret = strchr(std_src, c);
	printf("Test: %s\n", desc);
...
int	test_strrchr(char *src, char c, char *desc)
{
	...
	strncpy(ft_src, src, sizeof(ft_src));
	strncpy(std_src, src, sizeof(std_src));
	ft_ret = ft_strrchr(ft_src, c);
	std_ret = strrchr(std_src, c);
	printf("Test: %s\n", desc);
...
int	test_memchr(char *str, char c, int size, int print, char *desc)
{
	...
	ft_str = (char *)ft_memchr(str, c, size);
	std_str = (char *)memchr(str, c, size);
...
int	test_memcmp(char *str1, char *str2, int size, char *desc)
{
	...
	ft_res = normalize_result(ft_memcmp(str1, str2, size));
	std_res = normalize_result(memcmp(str1, str2, size));
	printf("%s\n", desc);
...
int	test_memset(char *src, char c, size_t size, char *desc)
{
	...
	ft_ret = ft_memset(ft_src, c, size);
	std_ret = memset(std_src, c, size);
	print_compare_array(ft_ret, std_ret, strlen(src), desc);
	if (memcmp(ft_ret, std_ret, size) != 0)
		return (0);
	return (1);
...
int	test_striteri(char *test_str, void (*f)(unsigned int, char *),
		char *expected, char *desc)
{
	char	*str;
...
int	test_strmapi(char *test_str, char (*f)(unsigned int, char), char *expected,
		char *desc)
{
	char	*str;
	...
	if (!str)
	{
		printf("Failed to allocate memory for str in strmapi");
		return (0);
	}
	printf("%s\n", desc);
	printf("Original string: %s\n", str);
...
int	test_substr(char *str, unsigned int start, size_t len, char *expected,
		char *desc)
{
	char	*substring;
...
int	test_strtrim(char *str, char *set, char *expected, char *desc)
{
	char	*my_str;

	printf("%s\n", desc);
	printf("trimming '%s' in '%s'\n", set, str);
	my_str = ft_strtrim(str, set);
	printf("my strtrim:	'%s'\nexpected:	'%s'\n", my_str, expected);
	if (strcmp(my_str, expected) == 0)
	{
		free(my_str);
...
int	main(void)
{
	...
	success *= test_bzero("TESTINGIT", 3, 10,
			"TESTINGIT but only first 3 bytes");
	...
	success *= test_memmove("Hello World!", 5, 0, 7,
			"Overlap backward memmove");
	...
	success = 1;
	// Test ft_strlcpy vs. strlcpy
	printf("\n-----ft_strlcpy vs. strlcpy------\n");
	success *= test_strlcpy("Hello", " World", 256, "Normal case");
	success *= test_strlcpy("Hello", " World", 3, "Size less than dest length");
	success *= test_strlcpy("Hello", " World", 0, "Size 0");
	success *= test_strlcpy("", " World", 256, "Empty dest");
	success *= test_strlcpy("Hello", "", 256, "Empty src");
	success *= test_strlcpy("", "", 256, "Both dest and src empty");
	success *= test_strlcpy("Hello", " World", 1, "Size 1");
	if (!success)
	{
		printf("Test failed for ft_strlcpy\n");
		strcat(fails, "ft_strlcpy, ");
	}
	success = 1;
	...
	success *= test_strchr("abracadabra", 'a',
			"Find a character occurring multiple times");
	if (!success)
	{
	...
	success *= test_strrchr("abcdefgh", 'e',
			"Find a character near the middle");
	success *= test_strrchr("abracadabra", 'a',
			"Find a character occurring multiple times");
	if (!success)
	{
	...
	success *= test_strncmp("abcde", "bbcde", 5,
			"Different at 1st char but compared up to 5 chars");
	...
	success *= test_strncmp("abc\0def", "abc\0dgh", 7,
			"Include null terminator in middle");
	success *= test_strncmp("abc\0def", "abc\0dgh", 4,
	...
	success *= test_strnstr("Hello, world!", "world", 13,
			"Found substring in the middle");
	success *= test_strnstr("Hello, world!", "universe", 13,
			"Substring not found");
	success *= test_strnstr("Hello, world!", "", 13, "Empty src");
	success *= test_strnstr("world", "Hello, world!", 5,
	...
	success *= test_strnstr("Hello, world!", "Hello", 20,
			"Len greater than dest length");
	success *= test_strnstr("Hello, world!", "wor", 9,
			"len too short for the full needle");
	...
	// Test ft_striteri
	printf("\n-----ft_striteri------\n");
	success *= test_striteri("hello world", cap_alternate, "HeLlO WoRlD",
			"cap all even characters in 'hello world'");
	if (!success)
	{
	...
	// Test ft_strmapi
	printf("\n-----ft_strmapi------\n");
	success *= test_strmapi("hello world", cap_even, "hElLo wOrLd",
	...
	// Test ft_substr
	printf("\n-----ft_substr------\n");
	success *= test_substr("hello world", 6, 5, "world",
			"Extract 'world' from 'hello world'");
	success *= test_substr("hello world", 0, 5, "hello",
			"Extract from the start of the string");
	success *= test_substr("hello world", 6, 5, "world",
			"Extract from the end of the string");
	success *= test_substr("hello world", 0, 11, "hello world",
	...
	success *= test_substr("hello", 0, 10, "hello",
			"Extract beyond the string's length");
	if (!success)
	{
	...
	// Test ft_split
	printf("\n-----ft_split------\n");
	success *= test_strsplit("test this thing now", ' ', expected1,
			"Basic test");
	success *= test_strsplit("   test this thing now", ' ', expected2,
			"Leading spaces test");
	success *= test_strsplit("test this thing now   ", ' ', expected3,
			"Trailing spaces test");
	success *= test_strsplit("test   this thing now", ' ', expected4,
